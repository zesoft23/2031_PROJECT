...
; This program collects 180 degrees of data and then transmits
; that data over serial to the web log.
...
Main: ; "Real" program starts here.
	...
	LOAD   Mask0
	OUT    SONAREN     ; enable left-facing sonar
	CALL   UARTNL
	CALL   UARTNL
	
; This program stores data in to a table in memory and then transmits
; all the data at once after the user presses PB1.
...
CheckDone:
; user can press PB1 to stop acquiring and send data
	IN     XIO         ; XIO contains KEYs
	AND    Mask0       ; KEY1 mask (KEY0 is reset and can't be read)
	JZERO  SendData    ; not done, start over
...
CheckAngle:
	...
	STORE  Temp
	JNEG   CheckDone   ; outside of table; don't acquire
	ADDI   -180
	JPOS   CheckDone   ; outside of table the other way
	ADDI   180
	ADDI   DataTable   ; offset with table starting address
	STORE  pointer
	
AcquireData: ; fill the memory with data
; for this example, robot will record the lowest value for
; each degree of rotation
	IN     Dist0       ; read the sonar distance
	OUT    SSEG2       ; debugging
	STORE  Temp
	ILOAD  pointer     ; get current saved value
	JZERO  SaveData    ; was 0, so definitely save
	SUB    Temp        ; compare with new data
	JNEG   CheckFill   ; new is farther, so don't save
SaveData:
	LOAD   Temp
	ISTORE pointer     ; save it in the table
	
	
CheckFill:
; out of curiosity, check how many entries in the table are
; non-zero (i.e. have received a value).
	LOADI  DataTable   ; get the table address
	STORE  pointer     ; set the pointer to start of table
	ADDI   180         ; find end of table (181 entries)
	STORE  Temp        ; save end for later
	LOADI  0
	STORE  Temp2
CFLoop:
	ILOAD  pointer     ; get table entry
	JZERO  CFEmpty     ; check if 0
	LOAD   Temp2       ; if not, increment counter
	ADDI   1
	STORE  Temp2
CFEmpty:
	LOAD   pointer
	XOR    Temp        ; check against table end address
	JZERO  CFEnd       ; if that was end, exit loop
	LOAD   pointer
	ADDI   1           ; otherwise...
	STORE  pointer     ; increment pointer
	JUMP   CFLoop      ; do it again
CFEnd:
	LOAD   Temp2
	OUT    SSEG1
...
; Because the UART is rate limited, we have to send one value
; at a time, with at least a 0.2s delay between each.
SendData:
	LOADI  0
...
SDLoop:
	CALL   UARTNL      ; newline
	ILOAD  pointer     ; get table entry
	CALL   UARTSend2   ; send two bytes
	LOAD   pointer
	XOR    Temp        ; check against table end address
	JZERO  SDEnd       ; if that was end, exit loop
	LOAD   pointer
	ADDI   1           ; otherwise...
	STORE  pointer     ; increment pointer
	AND    Mask0
	OUT    BEEP        ; user feedback for sending data
	LOADI  2
	CALL   WaitAC      ; wait for .2s for UART rate limit
	JUMP   SDLoop      ; do it again
SDEnd:
